...
def make_env(env_id: str, rank: int, seed: int = 0):
    ...
    Provides an isolated environment for each CPU core.
    """
    def _init():
        # 1. Create raw environment
        env = gym.make(env_id)

        # 2. Wrap with Monitor so SB3's internal logger can track ep_rew_mean & ep_len_mean
        env = Monitor(env)  # monitor raw reward

        # 3. Add StateCaptureWrapper to track raw scores and physics for our custom UI
        env = MlpStateCaptureWrapper(env)
        env = MlpCustomRewardWrapper(env)
        # env = Monitor(env)    # monitor shaped reward

        # Seed the environment for reproducibility, offset by rank to ensure diversity
...
class LoggingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if np.any(dones):
            all_raw_reward = self.training_env.get_attr("saved_raw_reward_sum")
            all_shaped_reward = self.training_env.get_attr("saved_shaped_reward_sum")
            all_steps = self.training_env.get_attr("saved_ep_step_count")

            for i, done in enumerate(dones):
                if done:
                    self.raw_reward_history.append(all_raw_reward[i])
                    self.shaped_reward_history.append(all_shaped_reward[i])
                    self.step_count_history.append(all_steps[i])
...
if __name__ == "__main__":
    ...
    MODEL_PREFIX = f"ppo_mlp_{current_time}_seed_{BASE_SEED}"
    ...
    vec_env = SubprocVecEnv([make_env(env_id, i, seed=BASE_SEED) for i in range(N_ENVS)])
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=max(int(TOTAL_TIMESTEPS / 20 / N_ENVS), 1),
        save_path=model_dir,
