...
import pygame
import numpy as np
from lunar_lander_wrappers import StateCaptureWrapper, CustomRewardWrapper

# ==========================================
# 1. Initialize Environment with Wrappers
# ==========================================
env = gym.make("LunarLander-v3", render_mode="rgb_array")
env_state = StateCaptureWrapper(env)
env = CustomRewardWrapper(env_state)
...
keys_to_action = {
    ...
    pygame.K_LEFT: 1,  # Left orientation engine
    pygame.K_RIGHT: 3  # Right orientation engine
}
...
while running:
    ...
    if not game_over:
        ...
        for key, act in keys_to_action.items():
            if keys[key]:
                action = act
                break

        current_action_text = action_map.get(action)
    ...
    if game_over:
        ...
        text_rect = result_surface.get_rect(center=(screen_width // 2, screen_height // 2 - 20))
        screen.blit(result_surface, text_rect)

        restart_msg = small_font.render("Press SPACE to try again", True, (200, 200, 200))
